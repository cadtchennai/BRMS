...
alert("Process Started in interview process!"+a.value);    
}
function myFunction1(processInstanceid) {
var a= document.getElementById("processInstanceId");
a.value=processInstanceid;
alert("Process Value in interview process!"+ a.value);
}
</script>
...
<td><input type="submit" name="action" value="Taken" id="complete${TaskBean.processInstanceId}"
		onclick="myFunctions(${TaskBean.processInstanceId})"></td> 
<td><input type="submit" name="action" value="Left" onclick="myFunction1(${TaskBean.processInstanceId});"></td>
</c:forEach>
</tbody>
</c:if>
</table>
